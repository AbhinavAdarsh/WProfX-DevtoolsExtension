...
class Analyze {
  ...
  analyzeTrace(traceEvents, /*viz,*/bar /*host*/){
    var modal = document.getElementById('myModal');
    //var bar = document.getElementById("myBar");
    var p1 =  this.processTrace(traceEvents)
    .catch(err => util.displayError(err));

    //console.log(traceEvents);
    var p2 = p1.then(this.sortEventsByTs())
    .catch(err => util.displayError(err));
    //console.log(JSON.stringify(this.networkTraceEvents, null, 4));
    //console.log(JSON.stringify(this.loadingTraceEvents, null, 4));
    var p3 = p2.then(this.processNetworkEvents(this.networkTraceEvents))
    .catch(err => util.displayError(err));
    var p4 = p3.then(this.processLoadingEvents(this.loadingTraceEvents))
    .catch(err => util.displayError(err));
    //console.log(traceEvents);

    var p5 = p4.then(this.processRenderingEvents(this.renderingTraceEvents))
    .catch(err => util.displayError(err));
    var p6 = p5.then(this.processPaintingEvents(this.paintingTraceEvents))
    .catch(err => util.displayError(err));

    var p7 = p6.then(this.findInterestingTimes())
    .catch(err => util.displayError(err));

    var p8 = p7.then(this.sortAllProcessedByStartTime())
    .catch(err => util.displayError(err));
    // console.log('this.network');
    // console.log(this.network);

    progressBar(bar, 70, 'Extracting dependency...');
    ...
    progressBar(bar, 90, 'Generating output...');
    window.setTimeout(function() {modal.style.display = "none"}, 300);
    var p11 = p10.then(this.timeSummary())
    .catch(err => util.displayError(err));

    var p11 = p10.then(this.orderLayout())
    .catch(err => util.displayError(err));

    var p11 = p10.then(this.writeOutputlog()).then(() => {
      // console.log('this.interactive');
      // console.log(this.interactive);
      //
      // console.log('Critical Path: ');
      // console.log(this.criticalPath);
      // console.log('this.output');
      // console.log(this.output);
      // console.log('this.startTime: ' + this.startTime );
      // console.log('this.endTime: ' + this.endTime );

      // console.log('this.lastActivity: ' + this.lastActivity);
      // console.log('this.depsParent');
      // console.log(this.depsParent);
      ...
      util.legendHandler();
      //util.metricLegendHandler();
      // util.depLineHandler();
      // util.criticalPathHandler();
      progressBar(bar, 100, 'Done.');
      window.setTimeout(function() {modal.style.display = "none"}, 500);
      util.InitForRerun();
    })
    .catch(err => util.displayError(err));
  }
    ...
    if (name == 'EvaluateScript' && ('ph' in trace_event && trace_event['ph']=='X')){
      if ('args' in  trace_event && 'data' in trace_event['args'] &&
          ...
          if (_id.length > 0){
            ...
            // console.log('orig , _id ' + trace_event['args']['data']['url']);
            // console.log(_id);
  ...
  processTraceEvents(){
    ...
    if (this.traceEvents.length >= 1){
      ...
      // console.log('this.traceEvents');
      // var _length = this.traceEvents.length;
      // for (i=0; i<_length;i++){
      //   console.log(this.traceEvents[i]);
      // }
  ...
  processTimelineTraceEvent(trace_event){
    ...
    if ((thread in this.threads) && ('dur' in trace_event ||
      ...
      if (trace_event['ph'] == 'E') {
        ...
        e = {'t': thread, 'n': this.eventNames[trace_event['name']],
             's': trace_event['ts']};
      ...
      if (e!= null && 'e' in e && e['s'] >= this.startTime && e['e'] >= e['s']){
        ...
        if (this.threadStack[thread].length > 0){
          parent = this.threadStack[thread].pop();
          if (!('c' in parent)){
            parent['c'] = [];
          }
          parent['c'].push(e);
  ...
  adjustTimelineSlice(thread, slice_number, name, parent, elapsed){
    try {
      ...
      if (name != parent){
        ...
        //console.log('thread, slice_number, name, parent, elapsed: ' + thread + ' ' + slice_number + ' ' + name + ' ' + parent + ' '  + elapsed);
        // for (t in this.cpu['slices'][thread]){
        //    console.log('this.cpu[slices][thread] ' + t);
        //  }
        // console.log('this.cpu[slices][thread][name] ' + this.cpu['slices'][thread][name]);
        // console.log('this.cpu[slices][thread][name][slice_number] ' + this.cpu['slices'][thread][name][slice_number]);
      ...
      for (var i=0; i<_length; i++){
        ...
        if (net_trace['name'] == 'ResourceSendRequest'){
          ...
          if ('stackTrace' in net_trace['args']['data']){
            var _script_url = net_trace['args']['data']['stackTrace'][0]['url'].split('#')[0];
  ...
  processLoadingEvents(loading_trace_events){
    return new Promise((resolve, reject) =>  {
      ...
      for (var i=0; i<_length2; i++) {
        ...
        if (load_list[i][0][0]['ph'] == 'B'){
          ...
          if ('stackTrace' in load_list[i][0][0]['args']['beginData']){
            var _scriptUrL = load_list[i][0][0]['args']['beginData']['stackTrace'][0]['url'].split('#')[0];
      ...
      //console.log(JSON.stringify(this.loading, null, 4));
  ...
  processRenderingEvents(rendering_trace_events){
    return new Promise((resolve, reject) =>  {
      ...
      for (var i=0; i<_length2; i++) {
        ...
        //console.log(JSON.stringify(this.rendering, null, 4));
  ...
  sortAllProcessedByStartTime(){
    return new Promise((resolve, reject) =>  {
      ...
      // console.log('scripts_main_thread');
      // console.log(scripts_main_thread);
        ...
        if (scripts_main_thread.hasOwnProperty(_id)){
          ...
          if ('EvaluateScript' in script_dict && script_dict['EvaluateScript'].length > 0){
            ...
            for (var j=0; j<_length2; j++) {
              this.scriptList.push([_id, {'startTime': script_dict['EvaluateScript'][j][0],
      ...
      // console.log('this.scriptXtra');
      // console.log(this.scriptXtra);
      // console.log('scriptIdsSoFar');
      // console.log(scriptIdsSoFar);
      // console.log('this.scriptXtra 951');
      // console.log(this.scriptXtra);
      ...
      // console.log('this.scriptLookupUrl');
      // console.log(this.scriptLookupUrl);
    ...
    for (var i=0; i<_length;i++){
      ...
      if (obj[0].startsWith('Network')){
        if (obj[1]['mimeType'] == 'text/html'){
          download_0 = obj;
          //this.startTime = obj[1]['startTime'];
          break;
        }
      }
    }
  ...
  findIdByUrl(_url, activitiy_data, _type){
    ...
    if (_type == 'network'){
      //console.log('In findIdByUrl: ' + _url);
      var _length = this.networkLookupUrl[_url].length;
      for (var i=0; i<_length; i++){
        var net_id = this.networkLookupUrl[_url][i];
          // var net_startTime = this.networkList[parseInt(net_id.split('_')[1])][1]['startTime'];
          // var net_endTime = this.networkList[parseInt(net_id.split('_')[1])][1]['endTime'];
        var net_startTime = this.allDict[net_id]['startTime'];
        // catch(e){
        //   console.log('in catch');
        //   console.log('net_id ' + net_id + 'url ' + _url + ' activitiy_data ' );
        //   console.log(this.networkLookupUrl);
        //   console.log(activitiy_data);
        //   throw 'Type error';
        //   }
      ...
      if (selected[0] == ''){
        // console.log('selected[0] == empty ' );
        // console.log(activitiy_data);
        // console.log(_url);
        // console.log(this.scriptLookupUrl[_url]);

        //console.log(this.networkList);
        //throw('In this.findIdByUrl(), script ' + this.scriptLookupUrl[_url] + _url );
  ...
  extractDependencies(){
    return new Promise((resolve, reject) =>  {
      ...
      //
      // console.log('In extractDependencies()');
      // console.log(this.download0Id);
      // console.log(this.parse0Id);
        ...
        if (_nodeId.startsWith('Networking') || _nodeId.startsWith('Loading') || _nodeId.startsWith('Scripting')){
          if (_nodeId.startsWith('Networking')){
            ...
            if (_nodeData['startTime'] > this.allDict[this.parse0Id]['startTime']){
              //console.log('_nodeData_startTime > this.allDict[this.parse0Id]_startTime for: ' + _nodeId);
            ...
            if (!(['Null', null, '', 'null'].includes(_nodeData['fromScript']))){
              ...
              var _parseId = this.findPrevParseId(_nodeData);
              // console.log('_parseId: ' + _parseId);
              // console.log('this.allDict[_parseID]');
              ...
              if (_nodeData['name'] == 'ParseAuthorStyleSheet'){
               ...
               var _script_nodeId = this.findScriptingId(_nodeData);
               if (!(_script_nodeId == null)){
                 //console.log('_script_nodeId is not null for ' + _nodeId);
  ...
  findCpMaxEnd(_array, _tailTime){
    ...
    // console.log('_array');
    // console.log(_array);
    // console.log('_tailTime: ' + _tailTime);
    for (var i=0; i<_length; i++){
      var _arr = _array[i];
      var parentId = _arr[0];
  ...
  writeOutputlog(){
    return new Promise((resolve, reject) =>  {
      ...
      for (var i=0; i<_length; i++){
        ...
        for (var j=0; j<_length2; j++){
          ...
          if (_nodeId.startsWith('Network')){
            ...
            _tmp_dict['activityId'] = _nodeId;
            //_tmp_merged_dict = Object.assign(_tmp_dict, this.loading[_nodeId]);
      ...
      //                       }
      var _tmp_critical_path = {'criticalPath': this.criticalPath,
  ...
  findInterestingTimes(){
    return new Promise((resolve, reject) =>  {
      ...
      console.log('this.firstMeaningfulPaintList');
      console.log(this.firstMeaningfulPaintList);
      ...
      console.log('this.firstMeaningfulPaint');
      console.log(this.firstMeaningfulPaint);
      console.log('this.domContentLoadedEventEnd');
      console.log(this.domContentLoadedEventEnd);
      console.log('this.loadEventEnd');
      console.log(this.loadEventEnd);
      console.log('this.firstContentfulPaint');
      console.log(this.firstContentfulPaint);
      console.log('this.firstPaint');
      console.log(this.firstPaint);
      console.log('this.mainFrameId');
      console.log(this.mainFrameId);
      console.log('this.timeToInteractive');
      console.log(this.timeToInteractive);
      resolve();
    });
  }
  ...
  timeSummary(){
    return new Promise((resolve, reject) =>  {
      ...
      this.networkingCp = util.round(this.networkingCp, 2);
      this.loadingCp = util.round(this.loadingCp, 2);
      this.scriptingCp = util.round(this.scriptingCp, 2);
      resolve();
    });
  }
//class ends
